...
const example_font_file = @embedFile("OpenSans-Bold.ttf");
...
test "create face from memory" {
    ...
    _ = try lib.createFaceMemory(example_font_file, 0);
}
...
test "load glyph" {
    const lib = try Library.init();
    const face = try lib.createFaceMemory(example_font_file, 0);
